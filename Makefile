...
.PHONY: all fetch verify load row segments blocks ors nrr validate export map shell clean clean-db
.DELETE_ON_ERROR:
...
export:   out/validation.txt out/narrow_blocks.txt out/ors_narrow_residential.txt out/nrr_convertible.txt out/portland_street_widths.csv out/portland_row_confident.csv

build:
	@mkdir -p build

out/map.html: sql/07_map_export.sql viz/template.html viz/build_map.py $(STAMP)/blocks | out build
